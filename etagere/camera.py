    ...
    for size in range(1, 15):
        ...
        for sharpness in [1, 0.5, 2, 0.2, 5]:
            ...
            if barcodes:
                ...
                os.makedirs("etagere/uploads/worked", exist_ok=True)
                os.rename(
                    image_path, f"etagere/uploads/worked/{os.path.basename(image_path)}"
                )
                return barcodes[0].data.decode("utf-8")
    os.makedirs("etagere/uploads/failed", exist_ok=True)
    os.rename(image_path, f"etagere/uploads/failed/{os.path.basename(image_path)}")

    # result = reader.readtext( f"etagere/uploads/failed/{os.path.basename(image_path)}", detail=0)
    # print(result)
    return None
